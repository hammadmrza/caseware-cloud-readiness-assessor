...
def generate_pdf():
    """Generate a professional PDF report of the assessment results."""
    pdf = FPDF()
    ...
    pdf.ln(30)
    pdf.cell(0, 12, "Caseware Cloud", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 12, "Migration Readiness Assessment", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(8)
    pdf.set_draw_color(45, 106, 159)
    ...
    pdf.cell(0, 8, "Desktop Working Papers to Caseware Cloud", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(20)

    pdf.set_font("Helvetica", "", 11)
    pdf.set_text_color(60, 60, 60)
    info_data = [
        ("Firm", firm_name if firm_name else "Not specified"),
        ("Firm Size", firm_size if firm_size != "Select..." else "Not specified"),
        ("Assessment Date", datetime.now().strftime("%B %d, %Y")),
        ("Prepared by", "Hammad Mirza"),
        ("Document Type", "Migration Readiness Assessment Report"),
    ]
    ...
    pdf.cell(0, 6, "This report is generated by the Caseware Cloud Migration Readiness Assessor.", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 6, "Not affiliated with or endorsed by Caseware International Inc.", align="C", new_x="LMARGIN", new_y="NEXT")
    ...
    pdf.cell(0, 10, "Overall Readiness Score", new_x="LMARGIN", new_y="NEXT")
    pdf.set_draw_color(45, 106, 159)
    ...
    pdf.cell(0, 18, f"{pct}%  -  {tier}", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)

    pdf.set_font("Helvetica", "", 11)
    pdf.set_text_color(60, 60, 60)
    pdf.multi_cell(0, 6, tier_desc)
    pdf.ln(4)
    pdf.set_font("Helvetica", "", 9)
    pdf.set_text_color(140, 140, 140)
    pdf.cell(0, 6, f"Score: {total_score} / {total_max} across 5 assessment categories", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(8)
    ...
    for cat, (s, m) in scores.items():
        p = round((s / m) * 100) if m > 0 else 0
        pdf.set_font("Helvetica", "", 10)
        pdf.set_text_color(60, 60, 60)
        pdf.cell(55, 7, cat, new_x="END")
        ...
        pdf.cell(0, 7, f"  {p}%", new_x="LMARGIN", new_y="NEXT")
        pdf.ln(2)
    ...
    if high_risks or med_risks:
        pdf.set_text_color(26, 60, 94)
        pdf.set_font("Helvetica", "B", 14)
        pdf.cell(0, 10, "Risk Flags", new_x="LMARGIN", new_y="NEXT")
        pdf.set_draw_color(45, 106, 159)
        pdf.line(10, pdf.get_y(), 200, pdf.get_y())
        pdf.ln(6)
    ...
    if strengths:
        ...
        pdf.set_text_color(26, 60, 94)
        pdf.set_font("Helvetica", "B", 14)
        pdf.cell(0, 10, "Strengths & Accelerators", new_x="LMARGIN", new_y="NEXT")
        pdf.set_draw_color(45, 106, 159)
        pdf.line(10, pdf.get_y(), 200, pdf.get_y())
        pdf.ln(6)
    ...
    pdf.set_text_color(26, 60, 94)
    pdf.set_font("Helvetica", "B", 14)
    pdf.cell(0, 10, "Recommended Migration Approach", new_x="LMARGIN", new_y="NEXT")
    pdf.set_draw_color(45, 106, 159)
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(6)
    ...
    pdf.set_text_color(45, 106, 159)
    pdf.cell(0, 8, approach, new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)
    ...
    for i, (phase_name, duration, description) in enumerate(phases):
        ...
        pdf.set_font("Helvetica", "B", 10)
        pdf.set_text_color(26, 60, 94)
        pdf.cell(0, 7, f"Phase {i+1}: {phase_name} ({duration})", new_x="LMARGIN", new_y="NEXT")
        pdf.set_font("Helvetica", "", 10)
        pdf.set_text_color(60, 60, 60)
        pdf.multi_cell(0, 6, description)
        pdf.ln(3)

    # Section: Engagement Sizing
    pdf.ln(4)
    if pdf.get_y() > 230:
        pdf.add_page()
    pdf.set_text_color(26, 60, 94)
    pdf.set_font("Helvetica", "B", 14)
    pdf.cell(0, 10, "Engagement Sizing Estimate", new_x="LMARGIN", new_y="NEXT")
    pdf.set_draw_color(45, 106, 159)
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(6)

    team_str = "1 Lead + 1 Associate" if size_multiplier <= 1.8 else "1 Lead + 2 Associates"
    sizing_data = [
        ("Estimated Duration", f"{est_weeks_low} - {est_weeks_high} weeks"),
        ("Consultant Days", f"{consultant_days - 10} - {consultant_days + 10} days"),
        ("Recommended Team", team_str),
    ]
    ...
    # Clean markdown from next_steps for PDF
    for i, step in enumerate(next_steps):
        clean = step.replace("**", "")
        pdf.set_font("Helvetica", "", 10)
        pdf.set_text_color(60, 60, 60)
        pdf.multi_cell(0, 6, f"{i+1}. {clean}")
        pdf.ln(2)
    ...
    pdf.cell(0, 5, "Generated by Caseware Cloud Migration Readiness Assessor | Hammad Mirza | April 2026", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 5, "This tool is not affiliated with or endorsed by Caseware International Inc.", align="C", new_x="LMARGIN", new_y="NEXT")

    return pdf.output()


pdf_filename = f"Migration_Readiness_Assessment_{firm_name.replace(' ', '_') if firm_name else 'Report'}_{datetime.now().strftime('%Y%m%d')}.pdf"
